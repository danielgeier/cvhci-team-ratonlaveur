...
void SkinModel::train(const cv::Mat3b& img, const cv::Mat1b& mask)
{
	...
	//cv::xphoto::balanceWhite(newImg, balancedImg, 0, 0, 255, 0, 255);
	...
	cv::Mat *tempSkin = &skinPixel;
	cv::Mat notSkinPixel = cv::Mat::zeros(1, 3, CV_32FC1);

	cv::Mat *tempnotSkin = &skinPixel;
	//if(false)
	try
	{
	...
	//	pimpl->skin->train(*tempSkin);

		// Train not-skin em
	//	pimpl->notSkin->train(*tempnotSkin);
...
cv::Mat1b SkinModel::classify(const cv::Mat3b& img)
{
	...
	//if (modeCount % 5 == 1)
	//{
		cv::FileStorage skinfs("testskin" + std::to_string(mode) + ".yml", cv::FileStorage::WRITE);
		cv::FileStorage notskinfs("testnonSkin" + std::to_string(mode) + ".yml", cv::FileStorage::WRITE);
